...
特に「オブジェクト型」や「null型」「undefined型」は、「オブジェクト値」「null値」「undefined値」と見た目が同じなので混同しやすいですが、別物です。
例えば、「null型」は「null値」のみをとることができる型です。
エディタ上だと色が違うので、色も意識してみてください。
...
`type` で `Lesson` 型を定義しています。
引数に `Lesson` 型を型注釈しているので、関数へ渡すデータ構造を統一できます。
上記の `printLesson` は `Lesson` 型のデータしか受け取れません。

## type を使おう

- 「データの構造に名前を付ける」ために `type` を使う
- まずは、よく使うオブジェクトから型を定義してみましょう
